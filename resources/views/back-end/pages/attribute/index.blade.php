									...
									<div class="card card-flush">
										<div class="card-header align-items-center py-5 gap-2 gap-md-5">
											<div class="card-toolbar flex-row-fluid justify-content-end gap-5">
												...
												<a href="{{ url("$segment/attribute/add") }}" class="btn btn-primary">Add</a>
											</div>
										</div>
													...
													<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
														<thead>
															<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
																...
																<th style="width:30%;" class="text-center">รูปภาพ</th>
																<th style="width:30%;" class="text-center">ชื่อ</th>
															...
															@if (@$items->count() > 0)
																@foreach (@$items as $index => $item)
																	<tr>     
																		...
																		<td class="text-center"> <img src="{{ asset('upload/Attribute/'.$item->img) }}" style="width:121px"
                                                                			class="img-fluid mt-3" /></td>
		...
		<script>
				...
				$(document).ready(function() {
					$('.update-status').on('change', function() {
						...
						$.ajax({
							url: 'webpanel/attribute/update-status',
							type: 'POST',
							data: {
								_token: '{{ csrf_token() }}',
								id: id,
			...
			function deleteItem(id) {
				Swal.fire({
					...
					preConfirm: () => {
						return fetch(window.location.origin + '/webpanel/attribute/destroy/'+id, {
