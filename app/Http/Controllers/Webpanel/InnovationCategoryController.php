...
class InnovationCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $navs = [
            ...
            '1' => ['url' => "$this->segment/innovation/category", 'name' => "หมวดหมู่", "last" => 1],
        ];
    ...
    public function status(Request $request, $id = null)
    {
        try {
            DB::beginTransaction();
             $data = InnovationCategory::find($id);
    ...
    public function add()
    {
        $navs = [
            '0' => ['url' => "javascript:void(0)", 'name' => "ศูนย์นวัตกรรม", "last" => 0],
            '1' => ['url' => "$this->segment/innovation/category", 'name' => "หมวดหมู่", "last" => 1],
            '2' => ['url' => "$this->segment/innovation/category/add", 'name' => "Add", "last" => 2],
        ];
    ...
    public function edit(Request $request, $id)
    {
        ...
        $navs = [
            '0' => ['url' => "javascript:void(0)", 'name' => "ศูนย์นวัตกรรม", "last" => 0],
            '1' => ['url' => "$this->segment/innovation/category", 'name' => "หมวดหมู่", "last" => 1],
            '2' => ['url' => "$this->segment/innovation/category/edit/$id", 'name' => "Edit", "last" => 2],
        ];
    ...
    { 
        $datas = InnovationCategory::find(explode(',', $id));
        if (@$datas) {
            foreach ($datas as $data) {
                $query = InnovationCategory::destroy($data->id);
            }
        }
    ...
    public function store($request, $id = null)
    {
        try {
            ...
            if ($data->save()) {
                DB::commit();
                return view("$this->prefix.alert.success", ['url' => url("$this->segment/innovation/category")]);
            } else {
                return view("$this->prefix.alert.error", ['url' => url("$this->segment/innovation/category")]);
            }
        } catch (\Exception $e) {
            DB::rollback();
