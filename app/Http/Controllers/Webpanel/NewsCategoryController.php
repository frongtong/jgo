...
class NewsCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $navs = [
            '0' => ['url' => "javascript:void(0)", 'name' => "หมวดหมู่กิจกรรม", "last" => 0],
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $datas = NewsCategory::find(explode(',', $id));
        if (@$datas) {
            foreach ($datas as $data) {
                $query = NewsCategory::destroy($data->id);
            }
        }
    ...
    public function store($request, $id = null)
    {
        try {
            ...
            if ($data->save()) {
                DB::commit();
                return view("$this->prefix.alert.success", ['url' => url("$this->segment/news_category")]);
            } else {
                return view("$this->prefix.alert.error", ['url' => url("$this->segment/news_category")]);
            }
        } catch (\Exception $e) {
            DB::rollback();
