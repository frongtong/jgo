...
class VoCategory2Controller extends Controller
{
    protected $segment = 'webpanel';
    protected $prefix = 'back-end';
    protected $folder = 'vocategory2';
    public function getCategory2($category1_id)
    {
        $category2 = VoCategory2::where('status','on')->where('category1_id', $category1_id)->get();
        return response()->json($VoCategory2);
    }
    ...
    public function index(Request $request, $category1_id)
    {
        ...
        $items = VoCategory2::query()->where('category1_id', $category1_id)->paginate(15);
        $items->pages = new VoCategory2();
    ...
    // {
    //     $VoCategory2 = VoCategory2::with('VoCategory2_link')->whereIn('id', $request->id)->get();

    //     if ($VoCategory2->isEmpty()) {
    //         return response()->json(false);
    //     }

    //     foreach ($VoCategory2 as $cat) {

    //         foreach ($cat->VoCategory2_link as $cat_image) {
    //             Storage::disk('public')->delete($cat_image->image);
    //         }

    //         $VoCategory2->product_image()->delete();
    //         $VoCategory2->delete();
    //     }
    ...
    {
        $id = $request->input('id');
        $VoCategory2 = VoCategory2::find($id);

        if ($VoCategory2) {

            foreach ($VoCategory2->VoCategory2_link as $cat_image) {
                Storage::disk('public')->delete($cat_image->image);
            }

            $VoCategory2->VoCategory2_link()->delete();
            $VoCategory2->delete();
    ...
    {
        try {
            $item = VoCategory2::find($request->id);
